...
import { api, ExtractedNeedsResponse, Mission } from '@/lib/api';
...
export type CreateMissionStudioProps = {
  embedded?: boolean;
  skipAuthRedirect?: boolean;
  onPublished?: (mission: Mission) => void;
};

export default function CreateMissionStudio({
  embedded = false,
  skipAuthRedirect = false,
  onPublished,
}: CreateMissionStudioProps) {
  ...
  useEffect(() => {
    ...
    if (!authLoading && !user) {
      router.push('/login?redirect=/missions/create');
    }
  }, [user, authLoading, router, skipAuthRedirect]);
  ...
  if (!skipAuthRedirect && (authLoading || !user)) {
    return (
  ...
  return (
    <div className={embedded ? 'w-full' : 'mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8'}>
      <div className={`mb-8 max-w-2xl ${embedded ? 'mb-6' : ''}`}>
        <h1
          className={`${
            embedded ? 'text-2xl sm:text-3xl' : 'text-3xl sm:text-4xl'
          } font-semibold text-[#0b1f3a] leading-tight ${locale === 'ar' ? 'font-cairo' : 'font-display'}`}
        >
      ...
      <div className="surface-panel rounded-xl p-6 sm:p-8 mb-8 border border-[#d8e0ea] bg-white">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          <div>
            ...
            <p className="text-xs text-[#5b6b7c] mt-0.5">{t('create_mission.registered_skills_subtitle')}</p>
          </div>
      ...
      {extractedData && (
        <div className="surface-panel rounded-xl p-6 sm:p-8 animate-fade-up">
          ...
          <div className="mb-4 text-sm font-semibold text-[#0b1f3a]">{t('create_mission.roles_title')}</div>
